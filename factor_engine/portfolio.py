"""
Portfolio-level Fama-French 3-factor analysis.
...
1. Combined return series (Tier 1 headline)
   ...
   individual holding log returns.  One joint FF3 regression on that combined
...
2. Per-holding attribution (Tier 2)
   FF3 is run independently on each holding.  Weighted-average contributions
...
rather than ETF proxies, for maximum accuracy.
"""
...
def _run_ff3_ols(
    ...
    """
    Fit: excess_return = α + β_mkt·mkt_excess + β_smb·smb + β_hml·hml + ε

    Returns a dict with all regression outputs.
    """
    X = add_constant(factors[["mkt_excess", "smb", "hml"]])
...
def generate_plain_english_summary(headline: dict, per_holding: list[dict]) -> str:
    ...
    lines += [
        "",
        f"Model fit: R² = {r2:.3f}  |  FF3 explains {r2 * 100:.1f}% of daily portfolio variance.",
...
def run_per_holding_regressions(
    ...
    for ticker, weight in weights.items():
        ...
        reg = _run_ff3_ols(excess, combined)
        reg.update({
            "ticker":           ticker,
            "weight":           weight,
