...
Methodology
-----------
We apply the portfolio's estimated FF3 betas to the *actual* factor returns
...
specific market/size/value loading structure would have produced under those
macro factor shocks.
...
def run_stress_tests(
    ...
    Parameters
    ----------
    beta_market, beta_smb, beta_hml : float
    ...
    for key, scenario in SCENARIOS.items():
        ...
        factors = get_ff3_daily(s, e)
        if factors.empty:
            print(f"  Warning: no factor data for {key}; skipping.")
            continue

        est = _estimate_scenario_return(factors, beta_market, beta_smb, beta_hml, alpha_daily)
